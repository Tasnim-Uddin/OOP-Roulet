...
class Wheel:
    ...
    def __init__(self):
        self.bins = list(Bin() for _ in range(38))  # Create 38 bins in a list
...
class PlayerCancellation(Player):
    """
    The player bets the sum of the first and last numbers in a list containing a series of numbers.
    If they win, they cancel the two numbers used ot make the bet.
    If they lose, the add the amount of the bet to the end of the sequence/list
...
class PlayerFibonacci(Player):
    """
    The player uses the Fibonacci sequence to bet.
    They first bet on 1.
    If they win they bet on 1 again, otherwise they bet on next value on Fibonacci sequence.
